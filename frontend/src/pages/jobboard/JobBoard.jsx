...
export default function JobBoard() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        setSelectedJob(JSON.parse(saved));
        // intentionally NOT setting modalOpen(true) here
      }
    } catch (err) {
  ...
  useEffect(() => {
    const handler = (e) => {
      if (e.key === "selectedJob") {
        setSelectedJob(e.newValue ? JSON.parse(e.newValue) : null);
      }
    };
  ...
  const handleCloseJob = () => {
    setModalOpen(false);
    // selectedJob stays — green border remains
  };

  const handleDeselectJob = (e) => {
    e.stopPropagation();
    setSelectedJob(null);
    setModalOpen(false);
    ...
    <div className="jobboard">
      ...
      <div className="jobboard__header">
        <div className="jobboard__header-left">
          ...
          {!connected && (
            <span className="jobboard__status">⚠ Reconnecting…</span>
          )}
        </div>

        <button
          className="jobboard__settings-btn"
